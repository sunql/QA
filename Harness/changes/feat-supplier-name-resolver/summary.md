...
### [2026-09-02] 裸公司名支持（后续优化 1/2 落地）

用户实测「济南吉利汽车有限公司」裸名（无「供应商」前缀）422。根因：名称正则要求前缀。
新增裸名回退路径：

- `_BARE_NAME_RE`：以 有限公司/有限责任公司 结尾、后缀前 ≥ 4 字符（数据驱动：
  真实供应商名最短前缀 4，2743/3500 带公司后缀），普通句子（「什么是有限公司」）
  被长度门槛过滤
- 无分隔符前缀写法（「评估供应商济南吉利汽车有限公司」）经裸名路径 + 关键字剥离恢复
- `apply()` 规范化替换：名称前紧邻「供应商」→ 只替换为编码；否则替换为
  「供应商 {code}」，保证下游 arg_extractor 数字正则可命中
- `MSG_AGENT_RUN_BAD_INPUT` 文案补充企业名称选项
- 单测：resolver 28 用例（新增裸名契约 6 条 + apply 规范化 3 条）；全量 1473 通过
- e2e：5 种名称输入形态（裸名/裸名句/无分隔符前缀/带前缀）全部 200 + enterpriseCode 10105
