...
SELECT
  ...
  -- 脏数量清洗：X3 PORDERQ 存在 19 行 order_qty > 1e9 的污染数据（源 qty 异常放大，如
  -- C12009POH0182 qty 达 4.1e17），GR 侧全库 MAX 仅 120 万，1e9 为天然分界；该行 qty 与金额置 NULL
  CASE WHEN QTYUOM_0 > 1000000000 THEN NULL ELSE QTYUOM_0 END,
  RCPQTYPUU_0,
  INVQTYPUU_0,
  RETQTYPUU_0,
  CPRPRI_0,
  CASE WHEN QTYUOM_0 > 1000000000 THEN NULL ELSE LINAMT_0 END,
  CASE WHEN QTYUOM_0 > 1000000000 THEN NULL ELSE LINATIAMT_0 END,
  NULLIF(NETCUR_0, ' '),
  LINSTA_0,
  LINTYP_0,
  NULLIF(PQHNUM_0, ' '),
