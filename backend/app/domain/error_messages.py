...
MSG_SCHEMA_CHAT_SUPPLIER_RISK = "供应商风险评估（仅 intent=supplier_risk 时填充）；由 SupplierRiskService 基于 SUPPLIER_RISK_SCORE 主路径 + 其他 3 个 feature fallback 合成。前端 MessageItem 按字段存在性路由到 SupplierRiskCard"
MSG_SCHEMA_CHAT_SUPPLIER_KEY_MISSING = "请在问题中提供 enterprise_key（如「供应商 100001 的 360° 视图」或「供应商 100001 的风险」）"
...
MSG_AGENT_RUN_BAD_INPUT = (
    ...
    "请在问题中提供企业编码（如「供应商 10105」）或带公司后缀的企业名称"
    "（如「济南吉利汽车有限公司」）——以「供应商」/「supplier」开头后接"
    "5-9 位编码，或直接给出以「有限公司」结尾的完整公司名即可。"
    "若不确定名称，可先在「供应商 360°」页面用搜索框按名称查找。"
)
